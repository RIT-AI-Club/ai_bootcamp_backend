...
send_student_notifications     = "${SEND_STUDENT_NOTIFICATIONS:-true}"
send_admin_notifications       = "${SEND_ADMIN_NOTIFICATIONS:-true}"
email_rate_limit_per_hour      = ${EMAIL_RATE_LIMIT_PER_HOUR:-50}
email_retry_attempts           = ${EMAIL_RETRY_ATTEMPTS:-3}
email_retry_delay_seconds      = ${EMAIL_RETRY_DELAY_SECONDS:-60}
min_instances                  = ${MIN_INSTANCES:-1}
max_instances                  = ${MAX_INSTANCES:-10}
cpu                            = "${CPU:-1}"
memory                         = "${MEMORY:-512Mi}"
